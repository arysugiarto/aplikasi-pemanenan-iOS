...
class CreateGradingViewModel: ObservableObject {
    @Published var createGrading: [GradingCreateEntity] = []
    
    private let context = PersistenceController.shared.container.viewContext
    
    private let createGradingUseCase: CreateGradingUseCase
    
    init(createGradingUseCase: CreateGradingUseCase) {
        self.createGradingUseCase = createGradingUseCase
        loadCreateGrading()
    }
    
    func loadCreateGrading() {
        createGrading = createGradingUseCase.getCreateGrading()
    }
    
    func addCreateGrading(noGrading: String, noOrderSale: String, location: String, date: Date ,species: String) {
        createGradingUseCase.addCreateGrading(noGrading: noGrading, noOrderSale: noOrderSale, location: location, date: date,species: species)
    ...
    func updateCreateGrading(id: UUID, noGrading: String, noOrderSale: String) {
        createGradingUseCase.updateCreateGrading(id: id, noGrading: noGrading, noOrderSale: noOrderSale)
    ...
    private func appendNewGrading(noGrading: String, noOrderSale: String, location: String, date: Date, species: String) {
        let context = PersistenceController.shared.context
        let newGrading = GradingCreateEntity(context: context)
        newGrading.id = UUID()
